...
def send_doctor_alert(
    ...
    """Send SMS alert to doctor when patient is RED or escalated."""
    if not settings.DOCTOR_PHONE:
        print("[Alert] DOCTOR_PHONE not set, skipping SMS alert")
        return False
    ...
    try:
        ...
        sms = client.messages.create(
            ...
            to=settings.DOCTOR_PHONE,
        )
        print(f"[Alert] Doctor SMS sent: {sms.sid}")
        return True
    except Exception as e:
        print(f"[Alert] Doctor SMS failed: {e}")
        return False